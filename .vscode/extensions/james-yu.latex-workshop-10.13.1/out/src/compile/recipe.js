...
async function build(rootFile, langId, buildLoop, recipeName) {
    ...
    if (tools?.map(tool => tool.command).includes('latexmk') && rootFile === lw_1.lw.root.subfiles.path && lw_1.lw.root.file.path) {
        await createAuxSubFolders(lw_1.lw.root.file.path);
    }
    else {
 ...
 * @param {string} rootFile - Path to the root LaTeX file.
 */
async function createAuxSubFolders(rootFile) {
    const rootDir = path_1.default.dirname(rootFile);
    let auxDir = lw_1.lw.file.getAuxDir(rootFile);
    if (!path_1.default.isAbsolute(auxDir)) {
        auxDir = path_1.default.resolve(rootDir, auxDir);
    }
    ...
    for (const file of lw_1.lw.cache.getIncludedTeX(rootFile)) {
        ...
        const fullAuxDir = path_1.default.resolve(auxDir, relativePath);
        ...
        try {
            const fileStat = await lw_1.lw.file.exists(fullAuxDir);
            if (!fileStat ||
                ...
                lw_1.lw.external.mkdirSync(fullAuxDir, { recursive: true });
            }
        }
        catch (e) {
