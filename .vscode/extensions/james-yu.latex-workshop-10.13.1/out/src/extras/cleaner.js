...
function globAllMultipleCwds(globs, cwds) {
    return unique(cwds.map(cwd => globAll(globs, cwd)).flat()).sort((a, b) => b.localeCompare(a));
}
...
async function cleanGlob(rootFile) {
    ...
    const auxdir = path.resolve(path.dirname(rootFile), lw_1.lw.file.getAuxDir(rootFile));
    ...
    const explicitFolders = globAllMultipleCwds(folderGlobsExplicit, [auxdir, outdir]);
    const explicitFoldersSet = new Set(explicitFolders);
    const filesOrFolders = globAllMultipleCwds(fileOrFolderGlobs, [auxdir, outdir]).filter(file => !explicitFoldersSet.has(file));
    // Remove files first
