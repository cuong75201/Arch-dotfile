 ...
 * (e.g., '.tex').
 */
function hasAlwaysRootExt(extname) {
    return lw_1.lw.constant.ACTIVE_ROOTFILE_EXT.includes(extname);
}
/**
...
function getAuxDir(texPath) {
    ...
    const configuration = vscode.workspace.getConfiguration('latex-workshop', lw_1.lw.file.toUri(texPath));
    const auxDir = configuration.get('latex.auxDir') || './';
    let result = undefined;
    if (auxDir === '%OUTDIR%') {
    ...
    const aux = utils.replaceArgumentPlaceholders(texPath, exports.file.tmpDirPath)(auxDir);
...
function getLangId(filename) {
    ...
    if (lw_1.lw.constant.TEX_EXT.includes(ext) || extraTeXExts.includes(ext)) {
        return 'latex';
    }
