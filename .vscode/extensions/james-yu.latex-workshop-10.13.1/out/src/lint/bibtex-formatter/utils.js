...
function getBibtexFormatConfig(scope) {
    ...
    const formatConfig = {
        tab: tabs,
        case: {
            field: config.get('bibtex-format.case.field'),
            type: config.get('bibtex-format.case.type')
        },
        left: leftright[0],
        right: leftright[1],
...
function bibtexSortByMonth(a, b, config) {
...
function bibtexFormat(entry, config) {
    ...
    const convertCase = config.case.field === 'lowercase' ? (str) => str.toLowerCase() : (str) => str.toUpperCase();
 ...
 */
function getFieldString(entry, fieldName, config) {
    if (latex_utensils_1.bibtexParser.isEntry(entry)) {
